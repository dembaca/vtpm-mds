...
if [[ ! -f "$PKI_DIR/devid-ca.pem" ]]; then
  openssl req -x509 -newkey rsa:2048 -nodes \
    ...
    -days 3650 \
    -subj "/CN=qemu-mds Lab DevID CA/O=qemu-mds"
fi
...
sudo cp -f "$PKI_DIR/devid-ca.pem" "$PKI_DIR/devid-ca-key.pem" /etc/prox-mds/
[[ -f "$PKI_DIR/ek-chain.pem" ]] && sudo cp -f "$PKI_DIR/ek-chain.pem" /etc/prox-mds/
sudo chmod 644 /etc/prox-mds/devid-ca.pem /etc/prox-mds/ek-chain.pem 2>/dev/null || true
sudo chmod 600 /etc/prox-mds/devid-ca-key.pem
echo "Lab PKI ready under $PKI_DIR and /etc/prox-mds"
