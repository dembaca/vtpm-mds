...
- New configuration setting `mds.require_vm_identity`, default true. When it is
  true, a caller the inventory could not bind is refused at every endpoint that
  reports instance data: the `/latest/meta-data/` tree, the
  `/latest/dynamic/instance-identity/` endpoints and `/latest/identity`.
...
- `instance-metadata`: `Derive The Instance Id From Inventory Or The Caller
  ...
  stating that `instanceId` honours `X-Forwarded-For`. A new requirement states
  the refusal itself, so the individual endpoint requirements keep describing
  what a bound caller is served.
...
- `imds/handlers.go`: `getInstanceID`, `getClientIP`, and a refusal check on
  every handler that reports instance data.
...
- Any guest reaching the service over IPv6 or loopback, which `vm-inventory`
  cannot bind at all: such a caller is served metadata today and is refused
  after this change unless `require_vm_identity` is set to false.
